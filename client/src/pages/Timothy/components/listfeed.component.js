...
class Feed extends Component {
    ...
    componentDidMount(){
      ...
      if(jwt === null){
        ...
        axios.post('http://localhost:5000/api/cache/updownstate', {'feedid' : this.props.feed._id }, headers)
      ...
      if(jwt === null){
       const { hide } = cogoToast.warn('Click to login & upvote/downvote.', {
        ...
        if(this.state.publisher == json.data.feed.user._id){
          cogoToast.error(`You cant ${e.target.name} your own feed!`, { hideAfter : 5 })
        }
        else {
        ...
        // sync with mongo
        axios.put('http://localhost:5000/feed/update', json.data.feed, headers)
          .then(res => { 
            console.log(res);
            //sync wtih redis
            axios.put('http://localhost:5000/cache/updownstate', {'feedid' : json.data.feed._id, upvoted : (this.state.upvoted ? 1 : 0) }, headers)
            .then(resp => this.setState(resp.data))
            .catch(err => console.log(err));  
    ...
    render() {
      let likeimgurl = this.state.upvoted ? '../../logos/liked.png' : '../../logos/like.png';
      let dislikeimgurl = this.state.downvoted ? '../../logos/disliked.png' : '../../logos/dislike.png';
        ...
        <div className="card">
          <div className="row">
            ...
            <div className="col-md-2 px-3">
              <div>
                <br/>
                <div> <input type="image" disabled={this.state.upvoted} onClick={this.handleUpvoteDownvote} name="upvote" src={likeimgurl}  alt="upvote" width="30" height="30" /><span ref={this.upvotes} style={{fontSize: '16px', 'fontWeight': 'bolder', 'verticalAlign':'6px'}}>&nbsp;&nbsp;&nbsp;{this.props.feed.upvotes}</span> </div>
                <div> <input type="image" disabled={this.state.downvoted} onClick={this.handleUpvoteDownvote} name="downvote" src={dislikeimgurl}  alt="downvote" width="30" height="30" /><span ref={this.downvotes} style={{fontSize: '16px', 'fontWeight': 'bolder', 'verticalAlign':'6px'}}>&nbsp;&nbsp;&nbsp;{this.props.feed.downvotes}</span><br/></div>   
              </div>
            </div>
          </div>
        </div>
...
export default class ListFeed extends Component {
  ...
  feedList() {  
    return this.state.feeds.map(currentfeed => {
      return <Feed feed={currentfeed} socket={this.props.actions} key={currentfeed._id}/>;
    })
  }
  render() {
    return (
