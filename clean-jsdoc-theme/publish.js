  ...
  if (items.length) {
    ...
    items.forEach(function(item) {
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        ...
        itemsNav +=
          '<li>' + linktoFn(item.longname, item.name.replace(/^module:/, ''))
...
function buildNav(members) {
  var title =
    (env && env.opts && env.opts.theme_opts && env.opts.theme_opts.title) ||
    'Learn MERN 文档'
  var isHTML = RegExp.prototype.test.bind(/(<([^>]+)>)/i)
  var nav
  if (!isHTML(title))
    nav =
      '<h2><a href="index.html"><div class="text">' + title + '</div></a></h2>'
  else {
  ...
  var search =
    env && env.opts && env.opts.theme_opts && env.opts.theme_opts.search
  ...
  var menu =
    (env && env.opts && env.opts.theme_opts && env.opts.theme_opts.menu) ||
    undefined
  ...
  if (members.globals.length) {
    ...
    if (!globalNav) {
      ...
      nav +=
        '<h3>' + linkto('global', 'Global') + '</h3><ul>' + globalNav + '</ul>'
    }
  }
...
function buildSearch() {
  var search =
    '<div class="search-box"><input type="text" placeholder="Search..." id="search-box" />'
...
function buildFooter() {
  var footer =
    (env && env.opts && env.opts.theme_opts && env.opts.theme_opts.footer) || ''
  return footer
}
...
function createDynamicStyleSheet() {
  var style_classes =
    (env &&
      env.opts &&
      env.opts.theme_opts &&
      env.opts.theme_opts.create_style) ||
    undefined
...
function createDynamicsScripts() {
  var scripts =
    (env &&
      env.opts &&
      env.opts.theme_opts &&
      env.opts.theme_opts.add_scripts) ||
    undefined
  return scripts
}
...
function returnPathOfScriptScr() {
  var script_path =
    (env &&
      env.opts &&
      env.opts.theme_opts &&
      env.opts.theme_opts.add_script_path) ||
    undefined
  return script_path
}
...
function returnPathOfStyleSrc() {
  var style_path =
    (env &&
      env.opts &&
      env.opts.theme_opts &&
      env.opts.theme_opts.add_style_path) ||
    undefined
  return style_path
}
...
function getMetaTagData() {
  var meta =
    (env && env.opts && env.opts.theme_opts && env.opts.theme_opts.meta) ||
    undefined
  return meta
}
